    ...
    for _ in range(duration):
        for frame in LOADING_FRAMES:
            try:
                ...
                # Best-effort animation — message may have been deleted or
                # the edit rate-limited; log at debug so it's not fully
                # silent, but don't let it interrupt the caller.
                logger.debug(f"animate_loading: edit_text failed, stopping animation: {e}")
                return
    ...
    if user_id in LOGIN_STATE:
        ...
        if "data" in state and "client" in state["data"]:
            try:
                ...
                logger.debug(f"login cancel: client disconnect failed: {e}")
    ...
    if text.strip().lower() == "❌ cancel":
        if "data" in state and "client" in state["data"]:
            try:
                ...
                logger.debug(f"login cancel (text): client disconnect failed: {e}")
        del LOGIN_STATE[user_id]
        await message.reply(
            "<b>❌ Login process cancelled. 😌</b>",
            parse_mode=enums.ParseMode.HTML,
